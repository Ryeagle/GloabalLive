...
- (void)configSubviews
{
    self.view.backgroundColor = GLColorRGB(0xe9ecef);
    [self.view addSubview:self.liveContainerView];
    
    [self.liveContainerView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.size.mas_equalTo(CGSizeMake(GL_SCREEN_WIDTH, GL_SCREEN_WIDTH * 9 / 16));
...
- (GLLivePlayView *)playView
{
    if (!_playView) {
        ...
        _playView.backgroundColor = GLLiveViewColor;
    }
    return _playView;
}
@end
